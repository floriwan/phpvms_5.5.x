        ...
        echo '<td align="left">';
                    ...
                    while ($slots <= $event->slot_limit):
                        ...
                        if(!$check2) {
                            ...
                            if($pilot->pilotid == Auth::$pilotid && Auth::LoggedIn() == true) {
                              echo ' <a href="'.SITE_URL.'/index.php/events/remove_signup?id='.$pilot->pilotid.'&event='.$event->id.'">- <i class="fa fa-times" aria-hidden="true"></i></a>';
                            }
                            echo '<br />';
                        }
                    ...
                    while ($slots <= $event->slot_limit):
                        ...
                        if(!$check2) {
                            if(Auth::LoggedIn() == true)
                              echo date('G:i', $slot_time).' - <a href="'.SITE_URL.'/index.php/events/signup?eid='.$event->id.'&pid='.Auth::$userinfo->pilotid.'&time='.date('G:i', $slot_time).'"> <i class="fa fa-paper-plane" aria-hidden="true"></i></a><br />';
                            $slots++;
                        }
                        else {
                            $pilot = PilotData::getPilotData($check2->pilot_id);
