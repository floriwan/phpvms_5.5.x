...
class vStaffListData extends CodonData {
	...
	{
		$sql = 'SELECT * FROM '.TABLE_PREFIX.'staff_levels
				ORDER BY `order` ASC';
	...
	public static function GetAllStaffInCat($cat_level)
	{
		...
		FROM '.TABLE_PREFIX.'staff_members s
	...
	{
		$sql = 'SELECT *
		FROM '.TABLE_PREFIX.'staff_members
	...
	public static function AddCategory($name, $order) {
        ...
        $sql = "INSERT INTO ".TABLE_PREFIX."staff_levels (name, `order`)
                VALUES('$name', $order)";
	...
	public static function UpdateCategory($id, $name, $order) {
		...
		$sql = "UPDATE ".TABLE_PREFIX."staff_levels
	...
	public static function AddStaff($pilotid, $level_id, $title, $titleabr, $email, $order) {
        ...
        $sql = "INSERT INTO ".TABLE_PREFIX."staff_members (`level_id`, `pilotid`, title, titleabr, email, `order`, since)
	...
	public static function UpdateStaff($id, $pilotid, $level_id, $title, $titleabr, $email, $order, $bio) {
		...
		$sql = "UPDATE ".TABLE_PREFIX."staff_members
	...
	{
		$sql = "DELETE FROM ".TABLE_PREFIX."staff_levels
				WHERE id=$id";
	...
	{
		$sql = "DELETE FROM ".TABLE_PREFIX."staff_members
				WHERE id=$id";
	...
	public static function GetOnlineStaff($minutes = '')
	{
		...
		$sql = "SELECT a.*, p.*
				FROM ".TABLE_PREFIX."staff_members a, ".TABLE_PREFIX."sessions s
				LEFT JOIN ".TABLE_PREFIX."pilots p ON p.pilotid=s.pilotid
